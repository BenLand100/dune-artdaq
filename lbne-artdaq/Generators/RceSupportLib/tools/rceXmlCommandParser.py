...
from rceCommandParser import RceCommandParser

class RceXmlCommandParser(RceCommandParser):
    
    def __init__(self):
        
        super(RceXmlCommandParser, self).__init__()
        ...
        self.sender_params = {
              'host'        : ['/config/DaqHost', str],
              'port'        : ['/config/DaqPort', int],
              'rate'        : ['/config/SendRate', float],
              'millislices' : ['/config/NumMillislices', int],
              'microslices' : ['/config/NumMicroslices', int],
              'adcmode'     : ['/config/AdcMode', int],
              'adcmean'     : ['/config/AdcMean', float],
              'adcsigma'    : ['/config/AdcSigma', float],
              }
    ...
    def parse_request(self, cmd_data):
        ...
        # TODO this response needs to be in XML format like RCE will produce
        if cmd_ok:
    ...
    def softReset_cmd(self, args):
        ...
        # On real RCE this resets counters etc
