...
void OnlineMonitoring::MonitoringData::GeneralMonitoring(RCEFormatter const& rceformatter,
  ...
  if (!fFilledRunData) {
	...
	fileName = file->GetName();
    	if ( !file->IsDirectory() && fileName.EndsWith(".root") && !fileName.BeginsWith("dune_r-") ) {
...
void OnlineMonitoring::MonitoringData::WriteMonitoringData(int run, int subrun, int eventsProcessed, TString const& imageType) {
  ...
  mainHTML << "<head><link rel=\"stylesheet\" type=\"text/css\" href=\"../../style/style.css\"><title>35t: Run " << run << ", Subrun " << subrun <<"</title></head>" << std::endl << "<body><a href=\"http://dune-dqm.fnal.gov\">" << std::endl << "  <div class=\"bannertop\"></div>" << std::endl << "</a>" << std::endl;
  ...
  for (auto& component : {"General","RCE","SSP","PTB"}) {
    ...
    *componentHTML[component] << "<head><link rel=\"stylesheet\" type=\"text/css\" href=\"../../../style/style.css\"><title>35t: Run " << run << ", Subrun " << subrun <<"</title></head>" << std::endl << "<body><a href=\"http://dune-dqm.fnal.gov\">" << std::endl << "  <div class=\"bannertop\"></div>" << std::endl << "</a></br>" << std::endl;;
  ...
  mf::LogInfo("Monitoring") << "Monitoring for run " << run << ", subRun " << subrun << " has been updated and is viewable at http://dune-dqm.fnal.gov/OnlineMonitoring/Run" << run << "Subrun" << subrun;
