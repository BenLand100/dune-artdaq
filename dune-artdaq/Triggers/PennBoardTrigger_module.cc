...
bool trig::PennBoardTrigger::filterOnCounterPayload(art::Handle<artdaq::Fragments> const & rawPTB){
  ...
  if(rawPTB.isValid()){
    ...
    for (size_t frag_index=0; frag_index < rawPTB->size(); ++frag_index){
	  ...
	  dune::PennMicroSlice::Payload_Counter* myCounterPayload = dune::reinterpret_cast_checked<dune::PennMicroSlice::Payload_Counter*>(payload_data);
    ...
    for (size_t frag_index=0; frag_index < rawPTB->size(); ++frag_index){
      ...
      for(uint32_t payload_index = 0; payload_index < n_frames; payload_index++){
        ...
        if(payload_type == dune::PennMicroSlice::DataTypeTrigger){
          ...
          dune::PennMicroSlice::Payload_Trigger *myTriggerPayload = dune::reinterpret_cast_checked<dune::PennMicroSlice::Payload_Trigger*>(payload_data);      
  ...
  if(rawPTB.isValid()){
    ...
    switch ( payload_type )
      ...
      case dune::PennMicroSlice::DataTypeCounter:
        ...
        //        myPTBCounter = dune::reinterpret_cast_checked<PTBCounter*>(payload_data);
        myCounterPayload = dune::reinterpret_cast_checked<dune::PennMicroSlice::Payload_Counter*>(payload_data);
      ...
      case dune::PennMicroSlice::DataTypeTrigger:
        ...
        //        myPTBTrigger = dune::reinterpret_cast_checked<PTBTrigger*>(payload_data);
        myTriggerPayload = dune::reinterpret_cast_checked<dune::PennMicroSlice::Payload_Trigger*>(payload_data);
