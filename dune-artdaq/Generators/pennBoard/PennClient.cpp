...
dune::PennClient::PennClient(const std::string& host_name, const std::string& port_or_service, const unsigned int timeout_usecs) :
socket_(io_service_),
...
exception_(false)
{
  ...
  DAQLogger::LogDebug("PennClient") << "dune::PennClient constructor";
#endif
  ...
  try
  {
    ...
    while ((endpoint_iter != end) && (socket_.is_open() == false))
    {
      ...
      // on completion
      socket_.async_connect(endpoint, boost::bind(&dune::PennClient::async_connect_handler, _1, &error));
...
void dune::PennClient::send_command(std::string const & command)
{
...
void dune::PennClient::send_config(std::string const & config)
{
#ifdef __PTB_DEVEL_MODE__
...
void dune::PennClient::send_xml(std::string const & xml_frag)
{
  ...
  if (doc == NULL) {
    ...
    try {
      DAQLogger::LogWarning("PennClient") << "Failed to parse XML response: [" << response <<  "] (length " << response.length() << "). There is almost certainly a problem with this run.";
    } catch (...) {
      set_exception(true);
    } // Swallow
  }
      ...
      if (cur_node->type == XML_ELEMENT_NODE) {
        if (std::string((const char*)(cur_node->name)) == "success")
        {
          ...
          if (std::string((const char*)(msgContent)) == "true") {
            ...
            try{
              DAQLogger::LogError("PennClient") << "Command failed execution on the PTB. Run is most likely going to fail.";
            } catch (...) {
              set_exception(true);
            } // swallow
          }
...
void dune::PennClient::set_deadline(void)
{
...
void dune::PennClient::check_deadline(void)
{
  ...
  deadline_.async_wait(boost::bind(&dune::PennClient::check_deadline, this));
}

void dune::PennClient::async_connect_handler(const boost::system::error_code& ec, boost::system::error_code* output_ec)
{
  *output_ec = ec;
}

void dune::PennClient::async_completion_handler(
...
std::size_t dune::PennClient::send(std::string const & send_str)
{
  ...
  boost::asio::async_write(socket_, boost::asio::buffer(send_str),
      boost::bind(&dune::PennClient::async_completion_handler, _1, _2, &error, &send_len));
...
// std::string dune::PennClient::receive(void)
// {
...
// 			boost::bind(&dune::PennClient::async_completion_handler, _1, _2, &error, &receive_length));
...
void dune::PennClient::set_param_(std::string const & name, std::string const & encoded_value, std::string const & type)
{
...
bool dune::PennClient::response_is_ack(std::string const & response, std::string const & command)
{
  bool is_ack = true;
