    ...
    run_start_time_(0)
{
  ...
  RECV_DEBUG(1) << "dune::PennDataReceiver constructor";
...
//  }
  ...
  receiver_thread_ = std::unique_ptr<std::thread>(new std::thread(&dune::PennDataReceiver::run_service, this));
}

dune::PennDataReceiver::~PennDataReceiver()
{
  RECV_DEBUG(1) << "dune::PennDataReceiver destructor";
  ...
  RECV_DEBUG(3) << "dune::PennDataReceiver destructor: receiver thread joined OK";

}

void dune::PennDataReceiver::start(void)
{
  RECV_DEBUG(1) << "dune::PennDataReceiver::start called";
  ...
  start_time_ = std::chrono::high_resolution_clock::now();
	...
	if (deadline_io_object_ == DataSocket)
	{
		...
		while (data_socket_.available() > 0)
		{
			...
			{
				DAQLogger::LogInfo("PennDataReceiver") << "dune::PennDataReceiver:start: client closed data socket connection during flush operation";
				break;
			}
			else if (ec)
			{
				DAQLogger::LogWarning("PennDataReceiver") << "dune::PennDataReceiver::start: got unexpected socket error flushing data socket: " << ec;
				break;
			}
		}
		if (flush_length > 0)
		{
		  ...
		  // from "Info" level to "Warning" level
			...
			DAQLogger::LogWarning("PennDataReceiver") << "dune::PennDataReceiver::start: flushed " << flush_length << " bytes stale data off open socket";
		}
		else
		{
			DAQLogger::LogDebug("PennDataReceiver") << "dune::PennDataReceiver::start: no stale data to flush off socket";
		}
	}
...
void dune::PennDataReceiver::stop(void)
{
  RECV_DEBUG(1) << "dune::PennDataReceiver::stop called";
...
//  const uint32_t stop_timeout_usecs = 5000000;
 // uint32_t max_timeout_count = stop_timeout_usecs / tick_period_usecs_;
  ...
  RECV_DEBUG(1) << "dune::PennDataRecevier::stop : last sequence id was " << (unsigned int)last_sequence_id_;

  DAQLogger::LogInfo("PennDataReceiver")  << "dune::PennDataReceiver::stop : received " << millislices_recvd_ << " millislices in "
      << elapsed_secs << " seconds, rate "
      << rate << " Hz";

}

void dune::PennDataReceiver::commit_empty_buffer(PennRawBufferPtr& buffer)
{
  empty_buffer_queue_.push(std::move(buffer));
}

size_t dune::PennDataReceiver::empty_buffers_available(void)
{
  return empty_buffer_queue_.size();
}

size_t dune::PennDataReceiver::filled_buffers_available(void)
{
  return filled_buffer_queue_.size();
}

bool dune::PennDataReceiver::retrieve_filled_buffer(PennRawBufferPtr& buffer, unsigned int timeout_us)
{
...
void dune::PennDataReceiver::release_empty_buffers(void)
{
...
void dune::PennDataReceiver::release_filled_buffers(void)
{

  if (filled_buffer_queue_.size() > 0) {
    DAQLogger::LogWarning("PennDataReceiver") << "Releasing " << filled_buffer_queue_.size() << " filled buffers";
  }
...
void dune::PennDataReceiver::run_service(void)
{
  RECV_DEBUG(1) << "dune::PennDataReceiver::run_service starting";
...
// GBcopy:   Make this a warning if we are not officially stopping the run
  ...
  {
     RECV_DEBUG(1) << "dune::PennDataReceiver::run_service stopping";
  } else {
...
void dune::PennDataReceiver::do_accept(void)
{
  RECV_DEBUG(5) << "dune::PennDataReceiver::do_accept starting";
...
void dune::PennDataReceiver::do_read(void)
{
    ...
    do
    {
        ...
        if (!buffer_available)
        {
           if ((buffer_retries > 0) && ((buffer_retries % buffer_retry_report_interval) == 0)) {
            DAQLogger::LogWarning("PennDataReceiver") << "dune::PennDataReceiver::receiverLoop: no buffers available on commit queue due to more data entering the PennDataReceiver than leaving it, retrying ("
           ...
           if (buffer_retries > max_retries) {
              if (!suspend_readout_.load()) {
                 if (!exception_.load() ) {
                   try {
                       DAQLogger::LogError("PennDataReceiver") << "dune::PennDataReceiver::receiverLoop: too many buffer retries, signalling an exception";
                   } catch (...) {
                     set_exception(true);
           ...
           if (buffer_retries > buffer_retry_report_interval) {
              RECV_DEBUG(1) << "dune::PennDataReceiver::receiverLoop: obtained new buffer after " << buffer_retries << " retries";
           }
        }
...
// This is the older code, which we hopefully won't want to go back to when the new stuff above works
    ...
    do
    {
      ...
      if (!buffer_available)
      {
        buffer_retries++;
        try {
          DAQLogger::LogError("PennDataReceiver") << "dune::PennDataReceiver::receiverLoop no buffers available on commit queue (buffers available : " << empty_buffers_available() << ")";
        } catch(DAQLogger::LogError::ExceptClass &e) {
  ...
  boost::asio::async_read(data_socket_,
    ...
    else
    {
      ...
      {
	try {
	  DAQLogger::LogError("PennDataReceiver") << "Got error on aysnchronous read from the socket: " << ec << " " << ec.message();
	} catch (...) {
	  set_exception(true);
	}
      }
...
void dune::PennDataReceiver::handle_received_data(std::size_t length)
{

  RECV_DEBUG(2) << "dune::PennDataReceiver::handle_received_data: Handling "
  ...
  case ReceiveMicroslicePayload:
	...
	try {
      ...
      if(split_ptr != nullptr) {
        if(remaining_size_ > dune::PennDataReceiver::remaining_buffer_size) {
	  try {
      ...
      if(this_overlap_ptr != nullptr) {
        if(overlap_size_ + this_overlap_size > dune::PennDataReceiver::overlap_buffer_size_) {
	  try {
...
void dune::PennDataReceiver::suspend_readout(bool await_restart)
{
...
void dune::PennDataReceiver::set_deadline(DeadlineIoObject io_object, unsigned int timeout_usecs)
{
...
void dune::PennDataReceiver::check_deadline(void)
{
  ...
  if (run_receiver_.load())
  {
    deadline_.async_wait(boost::bind(&dune::PennDataReceiver::check_deadline, this));
  }
  else
  {
...
std::string dune::PennDataReceiver::nextReceiveStateToString(NextReceiveState val)
{
  try {
...
std::string dune::PennDataReceiver::millisliceStateToString(MillisliceState val)
{
  try {
...
std::size_t dune::PennDataReceiver::nextReceiveStateToExpectedBytes(NextReceiveState val)
{
  switch(val)
  {
    case ReceiveMicrosliceHeader:
...
void dune::PennDataReceiver::validate_microslice_payload(void) {
