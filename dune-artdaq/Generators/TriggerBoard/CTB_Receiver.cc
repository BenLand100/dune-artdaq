...
int CTB_Receiver::_word_receiver() {
  ...
  dune::DAQLogger::LogInfo("CTB_Receiver") << "Header size: " << header_size << std::endl
  ...
  if ( _error_state.load() ) {
    ...
    if ( closing_error ) {
      dune::DAQLogger::LogError("CTB_Receiver") << "Error in shutdown: " << closing_error.message() << std::endl ;
    }
  }

  socket.close( closing_error ) ;
  if ( closing_error ) {
    dune::DAQLogger::LogError("CTB_Receiver") << "Socket closing failed: " << closing_error.message() << std::endl ;
  }
...
bool CTB_Receiver::IsTSWord( const ptb::content::word::word_t & w ) noexcept {
...
bool CTB_Receiver::IsFeedbackWord( const ptb::content::word::word_t & w ) noexcept {
...
bool CTB_Receiver::_read( T & obj , boost::asio::ip::tcp::socket & socket ) {
  ...
  if ( receiving_error == boost::asio::error::eof) {
    ...
    dune::DAQLogger::LogInfo("CTB_Receiver") << "Socket closed: "<< receiving_error.message()  << std::endl ;
    return false ;
  }
  
  if ( receiving_error ) {
    dune::DAQLogger::LogError("CTB_Receiver") <<"Read failure: " << receiving_error.message() << std::endl ;
    return false ;
  }
