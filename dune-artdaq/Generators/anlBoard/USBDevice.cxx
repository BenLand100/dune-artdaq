...
void SSPDAQ::USBDevice::Open(bool slowControlOnly){
  ...
  if(FT_OpenEx(fCommChannel.SerialNumber,FT_OPEN_BY_SERIAL_NUMBER, &(fCommChannel.ftHandle)) != FT_OK){
    try {
      dune::DAQLogger::LogError("SSP_USBDevice")<<"Error opening USB comm path"<<std::endl;
    } catch (...) {}
  ...
  if (hasFailed) {								 
    ...
    try {
      dune::DAQLogger::LogError("SSP_USBDevice")<<"Error configuring USB comm channel parameters!"<<std::endl;
    } catch (...) {}
    FT_Close(commHandle);
    throw(SSPDAQ::EFTDIError("Failed to configure USB comm channel"));
  }
  dune::DAQLogger::LogInfo("SSP_USBDevice")<<"Opened control path"<<std::endl;
  if(slowControlOnly){
    dune::DAQLogger::LogInfo("SSP_USBDevice")<<"Device open!"<<std::endl;
    isOpen=true;
    return;
  }
  ...
  if (FT_OpenEx(fDataChannel.SerialNumber,FT_OPEN_BY_SERIAL_NUMBER, &(fDataChannel.ftHandle)) != FT_OK) {
    ...
    try {
      dune::DAQLogger::LogError("SSP_USBDevice")<<"Error opening USB data path"<<std::endl;
    } catch (...) {}
    FT_Close(commHandle);
  ...
  dune::DAQLogger::LogInfo("SSP_USBDevice")<<"Opened data path"<<std::endl;
  ...
  // Setup parameters for data path
    ...
    if (hasFailed) {
      ...
      FT_Close(commHandle);
      try {
	dune::DAQLogger::LogError("SSP_USBDevice")<<"Error configuring USB data channel parameters"<<std::endl;
      } catch (...) {}
...
void SSPDAQ::USBDevice::Close(){
  ...
  // to avoid crashing DUNEWare when Disconnect is pressed
  if(!fSlowControlOnly){
  ...
  if(FT_Close(fCommChannel.ftHandle)!=FT_OK){
    try {
      dune::DAQLogger::LogError("SSP_USBDevice")<<"Failed to close USB comm channel"<<std::endl;
    } catch (...) {}
  ...
  if(!fSlowControlOnly){
    if(FT_Close(fDataChannel.ftHandle)!=FT_OK){
      try {
	dune::DAQLogger::LogError("SSP_USBDevice")<<"Failed to close USB data channel"<<std::endl;
      } catch (...) {}
      throw(SSPDAQ::EFTDIError("Failed to close data channel"));
    }  
  }
  isOpen=false;
  dune::DAQLogger::LogInfo("SSP_USBDevice")<<"Device closed"<<std::endl;
} 
...
void SSPDAQ::USBDevice::DeviceQueueStatus(unsigned int* numWords)
{
  ...
  if(FT_GetQueueStatus(fDataChannel.ftHandle, &numBytes)!=FT_OK){
    try {
    dune::DAQLogger::LogError("SSP_USBDevice")<<"Error getting queue length from USB device"<<std::endl;
    } catch (...) {}
...
void SSPDAQ::USBDevice::DeviceReceive(std::vector<unsigned int>& data, unsigned int size){
  ...
  if(FT_Read(fDataChannel.ftHandle, (void*)buf, size*sizeof(unsigned int), &dataReturned)!=FT_OK){
    delete[] buf;
    try {
      dune::DAQLogger::LogError("SSP_USBDevice")<<"FTDI fault on data receive"<<std::endl;
    } catch (...) {}
				   ...
				   unsigned int txSize, unsigned int rxSizeExpected, unsigned int retryCount)
{
  ...
  while(!success){
	...
	DevicePurgeComm();
	++timesTried;
	dune::DAQLogger::LogWarning("SSP_USBDevice")<<"Send/receive failed "<<timesTried<<" times on USB link, retrying..."<<std::endl;
      }
      else{
	try {
	dune::DAQLogger::LogError("SSP_USBDevice")<<"Send/receive failed on USB link, giving up."<<std::endl;
	} catch (...) {}
	throw;
      }
    }
...
void SSPDAQ::USBDevice::SendUSB (SSPDAQ::CtrlPacket& tx, unsigned int txSize)
{
	...
	if(FT_Write(fCommChannel.ftHandle, (char*)&tx, txSize, &txSizeWritten)!=FT_OK
	  ...
	  try {
	    dune::DAQLogger::LogError("SSP_USBDevice")<<"Failed to send data on USB comm channel!"<<std::endl;
	  } catch (...) {}
...
void SSPDAQ::USBDevice::ReceiveUSB (SSPDAQ::CtrlPacket& rx, unsigned int rxSizeExpected)
{
	...
	if(errorCode!=FT_OK
	  ...
	  try {
	    dune::DAQLogger::LogError("SSP_USBDevice")<<"Failed to receive data on USB comm channel!"<<std::endl;
	  } catch (...) {}
...
void SSPDAQ::USBDevice::DevicePurge(FT_DEVICE_LIST_INFO_NODE& channel){
  ...
  if(hasFailed){
    try {
      dune::DAQLogger::LogError("SSP_USBDevice")<<"Error purging USB device queue"<<std::endl;
    } catch (...) {}
