/*
 * RceDataReceiver.cpp - RCE data recevier classed based on the Boost
 *asynchronous IO network library
 *
...
dune::RceDataReceiver::RceDataReceiver(
    ...
    uint32_t tick_period_usecs, uint16_t receive_port,
    ...
    : instance_name_(instance_name),
      ...
      number_of_microslices_per_millislice_(
          number_of_microslices_per_millislice),
      run_receiver_(true),
  ...
  // Initialise and start the persistent deadline actor that handles socket
  // operation
  ...
  receiver_thread_ = std::unique_ptr<std::thread>(
      new std::thread(&dune::RceDataReceiver::run_service, this));
}
...
dune::RceDataReceiver::~RceDataReceiver() {
  ...
  RECV_DEBUG(1)
      << "dune::RceDataReceiver destructor: receiver thread joined OK";
}
    ...
    while (data_socket_.available() > 0) {
      ...
      if (ec == boost::asio::error::eof) {
        DAQLogger::LogInfo(instance_name_)
            << "dune::RceDataReceiver:start: client closed data socket "
               "connection during flush operation";
        break;
      } else if (ec) {
        DAQLogger::LogWarning(instance_name_) << "dune::RceDataReceiver::start:"
                                                 " got unexpected socket error "
                                                 "flushing data socket: " << ec;
        break;
      }
    }
    if (flush_length > 0) {
      ...
      DAQLogger::LogWarning(instance_name_)
          << "dune::RceDataReceiver::start: flushed " << flush_length
          << " bytes stale data off open socket";
    } else {
      DAQLogger::LogDebug(instance_name_)
          << "dune::RceDataReceiver::start: no stale data to flush off socket";
    }
  }

  // Clean up current buffer state of any partially-completed readouts in
  // previous runs
  if (current_raw_buffer_ != nullptr) {
    RECV_DEBUG(0) << "dune::RceDataReceiver::start: dropping unused or "
                     "partially filled buffer containing " << microslices_recvd_
                  << " microslices";
...
void dune::RceDataReceiver::stop(void) {
    ...
    if (timeout_count > max_timeout_count) {
      ...
      try {
        DAQLogger::LogError(instance_name_)
            << "Timeout waiting for RceDataReceiver thread to suspend readout";
      }
      catch (...) {
      }
      break;
  ...
  RECV_DEBUG(0) << "dune::RceDataRecevier::stop : last sequence id was "
                << last_sequence_id_;
  RECV_DEBUG(0) << "dune::RceDataReceiver::stop : received "
                << millislices_recvd_ << " millislices in " << elapsed_secs
                << " seconds, rate " << rate << " Hz";
}
...
bool dune::RceDataReceiver::retrieve_filled_buffer(RceRawBufferPtr& buffer,
                                                   unsigned int timeout_us) {
  bool buffer_available = true;

  // If timeout is specified, try to pop the buffer from the queue and time out,
  // otherwise
  // block waiting for the queue to contain a buffer
  if (timeout_us == 0) {
    buffer = filled_buffer_queue_.pop();
  } else {
    buffer_available = filled_buffer_queue_.try_pop(
        buffer, std::chrono::microseconds(timeout_us));
  }
...
void dune::RceDataReceiver::do_accept(void) {
  ...
  acceptor_.async_accept(accept_socket_, [this](boost::system::error_code ec) {
    if (!ec) {
      RECV_DEBUG(1) << "Accepted new data connection from source "
                    << accept_socket_.remote_endpoint();
      ...
      if (ec == boost::asio::error::operation_aborted) {
        ...
        DAQLogger::LogError(instance_name_)
            << "Got error on asynchronous accept: " << ec;
      }
      this->do_accept();
    }
  });
...
void dune::RceDataReceiver::do_read(void) {
  ...
  if (millislice_state_ == MillisliceEmpty) {
    ...
    do {
      buffer_available = empty_buffer_queue_.try_pop(
          current_raw_buffer_, std::chrono::milliseconds(100));
      if (!buffer_available) {
        if ((buffer_retries > 0) &&
            ((buffer_retries % buffer_retry_report_interval) == 0)) {
          DAQLogger::LogWarning(instance_name_)
              << "dune::RceDataReceiver::receiverLoop: no buffers available on "
                 "commit queue due to more data entering the RceDataReceiver "
                 "than leaving it, retrying (" << buffer_retries
              << " attempts so far)";
        }
        buffer_retries++;
        ...
        if (buffer_retries > max_buffer_attempts_) {
          if (!suspend_readout_.load()) {
            if (!exception_.load()) {
              try {
                DAQLogger::LogError(instance_name_)
                    << "dune::RceDataReceiver::receiverLoop: too many buffer "
                       "retries, signalling an exception";
              }
              catch (...) {
                set_exception(true);
              }
        ...
        if (buffer_retries > buffer_retry_report_interval) {
          RECV_DEBUG(1) << "dune::RceDataReceiver::receiverLoop: obtained new "
                           "buffer after " << buffer_retries << " retries";
        }
      }
      ...
      if (suspend_readout_.load()) {
        DAQLogger::LogInfo(instance_name_)
            << "Suspending readout during buffer retry loop";
        this->suspend_readout(true);

        // When we come out of suspended at this point, the main thread will
        // have invalidated the current buffer, so set
        // buffer_available to false so that we get a fresh one for the next
        // read
    ...
    if (buffer_available) {
      ...
      RECV_DEBUG(2) << "Receiving new millislice into raw buffer at address "
                    << current_write_ptr_;
    } else {

      // TCN, Dec-04-15

      // Note this else clause is now technically redundant as the buffer retry
      // loop will only exit either
      // with a valid buffer or because the receiver is being shut down. I have
      // left the code in for now
      // since it demonstrates how to swallow the exception and signal at run
      // end, which may become
      // necessary again

      try {
        DAQLogger::LogError(instance_name_) << "Failed to obtain new raw "
                                               "buffer for millislice, "
                                               "terminating receiver loop";
      }
      catch (...) {
        ...
        if (!suspend_readout_.load()) {
          DAQLogger::LogInfo(instance_name_)
              << "Setting exception to true; suspend_readout is false";
          set_exception(true);
        }
      }
  ...
  RECV_DEBUG(3) << "RECV: state " << (unsigned int)next_receive_state_
                ...
                << " uslice " << microslices_recvd_ << " uslice size "
                << microslice_size_recvd_ << " mslice size "
                << millislice_size_recvd_ << " addr " << current_write_ptr_
      ...
      [this](boost::system::error_code ec, std::size_t length) {
        if (!ec) {
          ...
          if (ec == boost::asio::error::eof) {
            ...
            try {
              DAQLogger::LogError(instance_name_)
                  << "Got error on aysnchronous read: " << ec << "\n"
...
void dune::RceDataReceiver::handle_received_data(std::size_t length) {
  ...
  switch (next_receive_state_) {
	...
	DAQLogger::LogWarning(instance_name_)
        ...
        if (sequence_id_initialised_ &&
            (sequence_id != last_sequence_id_ + 1)) {
          DAQLogger::LogWarning(instance_name_)
              << "Got mismatch in microslice sequence IDs! Got " << sequence_id
	  ...
	  /*
        ...
        next_receive_state_ = ReceiveMicrosliceHeader;
        next_receive_size_ =
            sizeof(RceMicrosliceHeader) - microslice_size_recvd_;
        RECV_DEBUG(2) << "Incomplete header received for microslice "
                      << microslices_recvd_ << " size "
                      << microslice_size_recvd_ << " next recv size is "
                      << next_receive_size_;
      }
      break;
    ...
    case ReceiveMicroslicePayload:
      ...
      if (microslice_size_recvd_ == microslice_size_) {
        RECV_DEBUG(2) << "Complete payload received for microslice "
                      << microslices_recvd_;
        microslices_recvd_++;
        microslice_size_recvd_ = 0;
        millislice_state_ = MillisliceIncomplete;
        ...
        next_receive_size_ = sizeof(RceMicrosliceHeader);
        current_header_ptr_ =
            (void*)((char*)current_header_ptr_ + microslice_size_);
      } else {
        RECV_DEBUG(2) << "Incomplete payload received for microslice "
                      << microslices_recvd_ << " (got "
                      << microslice_size_recvd_ << " expected "
                      << microslice_size_ << " bytes)";
        millislice_state_ = MicrosliceIncomplete;
        next_receive_state_ = ReceiveMicroslicePayload;
    ...
    case BlowOff:
      // something went wrong with the header...try to loop through the data
      // until we get the safeword
      thisWord = *(uint32_t*)(current_write_ptr_);
      if (thisWord == safeWord) {
        DAQLogger::LogWarning(instance_name_)
            << "Found the safeword! after this many bytes "
            << microslice_size_recvd_;
        microslices_recvd_++;
        current_header_ptr_ =
            (void*)((char*)current_header_ptr_ + microslice_size_recvd_);
        microslice_size_recvd_ = 0;
        millislice_state_ = MillisliceIncomplete;
        next_receive_state_ = ReceiveMicrosliceHeader;
        next_receive_size_ = sizeof(RceMicrosliceHeader);
      } else {
        // DAQLogger::LogWarning(instance_name_) << "Blowing off! Word is this:
        // 0x" << std::hex <<  thisWord<<std::dec<<"  after
        // "<<microslice_size_recvd_<<" words...safeWord = "<<std::hex
        // <<safeWord<<std::dec;
        millislice_state_ = MicrosliceIncomplete;
        next_receive_state_ = BlowOff;
        next_receive_size_ = sizeof(uint32_t);
    ...
    default:
      ...
      DAQLogger::LogError(instance_name_)
          << "Fatal error after async_recv - unrecognised next receive state: "
          << next_receive_state_;
      return;
      break;
  }
  ...
  // If correct number of microslices have been received, flag millislice as
  // complete
  if (microslices_recvd_ == number_of_microslices_per_millislice_) {
    RECV_DEBUG(1) << "Millislice " << millislices_recvd_ << " complete with "
                  << microslices_recvd_ << " microslices, total size "
                  << millislice_size_recvd_ << " bytes";
    millislices_recvd_++;
    millislice_state_ = MillisliceComplete;
  }

  // If the millislice is complete, place the buffer to the filled queue and set
...
void dune::RceDataReceiver::suspend_readout(bool await_restart) {
  ...
  if (await_restart) {
    RECV_DEBUG(2)
        << "TpcRceReceiver::suspend_readout: awaiting restart or shutdown";
    while (suspend_readout_.load() && run_receiver_.load()) {
      usleep(tick_period_usecs_);
    }
    RECV_DEBUG(2) << "TpcRceReceiver::suspend_readout: restart or shutdown "
                     "detected, exiting wait loop";
  }
}

void dune::RceDataReceiver::set_deadline(DeadlineIoObject io_object,
                                         unsigned int timeout_usecs) {

  // Set the current IO object that the deadline is being used for
  deadline_io_object_ = io_object;

  // Set the deadline for the asynchronous write operation if the timeout is
  // set, otherwise
...
void dune::RceDataReceiver::check_deadline(void) {
  ...
  if (run_receiver_.load()) {
    deadline_.async_wait(
        boost::bind(&dune::RceDataReceiver::check_deadline, this));
  } else {
