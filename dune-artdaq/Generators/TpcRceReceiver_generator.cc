...
dune::TpcRceReceiver::TpcRceReceiver(fhicl::ParameterSet const& ps)
  ...
  instance_name_for_metrics_ =
      "RCE " + boost::lexical_cast<std::string>(board_id);

  empty_buffer_low_water_metric_name_ =
      instance_name_for_metrics_ + " Empty Buffer Low Water Mark";
  empty_buffer_available_metric_name_ =
      instance_name_for_metrics_ + " Empty Buffers Available";
  filled_buffer_high_water_metric_name_ =
      instance_name_for_metrics_ + " Filled Buffer High Water Mark";
  filled_buffer_available_metric_name_ =
      instance_name_for_metrics_ + " Filled Buffers Available";
  ...
  dpm_client_host_port_ = ps.get<std::string>("rce_client_host_port", "9999");
  dpm_client_timeout_usecs_ = ps.get<uint32_t>("rce_client_timeout_usecs", 0);

  dtm_client_enable_ = ps.get<bool>("dtm_client_enable", false);
  dtm_client_host_addr_ =
      ps.get<std::string>("dtm_client_host_addr", "localhost");
  dtm_client_host_port_ = ps.get<std::string>("dtm_client_host_port", "9999");
  dtm_client_timeout_usecs_ = ps.get<uint32_t>("dtm_client_timeout_usecs", 0);
  ...
  rce_hls_blowoff_ = ps.get<uint32_t>("rce_hls_blowoff", 0);

  rce_data_dest_host_ = ps.get<std::string>("rce_data_dest_host", "127.0.0.1");
  rce_data_dest_port_ = ps.get<uint16_t>("rce_data_dest_port", 8989);
  rce_data_num_millislices_ = ps.get<uint32_t>("rce_data_num_millislices", 10);
  rce_data_num_microslices_ = ps.get<uint32_t>("rce_data_num_microslices", 10);
  rce_data_frag_rate_ = ps.get<float>("rce_data_frag_rate", 10.0);
  rce_data_adc_mode_ = ps.get<uint16_t>("rce_data_adc_mode", 0);
  rce_data_adc_mean_ = ps.get<float>("rce_data_adc_mean", 1000.0);
  rce_data_adc_sigma_ = ps.get<float>("rce_data_adc_sigma", 100.0);

  receive_port_ = ps.get<uint16_t>("receive_port", 9999);
  raw_buffer_size_ = ps.get<size_t>("raw_buffer_size", 10000);
  raw_buffer_precommit_ = ps.get<uint32_t>("raw_buffer_precommit", 10);
  ...
  int receiver_debug_level = ps.get<int>("receiver_debug_level", 0);
  ...
  reporting_interval_time_ = ps.get<uint32_t>("reporting_interval_time", 0);

#ifndef NO_RCE_CLIENT

  // Create a client connection to the DPM
  dpm_client_ = std::unique_ptr<dune::RceClient>(
      new dune::RceClient(instance_name_, dpm_client_host_addr_,
                          dpm_client_host_port_, dpm_client_timeout_usecs_));

#endif

  // Create a RceDataReceiver instance
  data_receiver_ =
      std::unique_ptr<dune::RceDataReceiver>(new dune::RceDataReceiver(
          instance_name_, receiver_debug_level, receiver_tick_period_usecs_,
          receive_port_, number_of_microslices_per_millislice_,
          max_buffer_attempts_));
}
...
void dune::TpcRceReceiver::start(void) {
  ...
  // Pre-commit buffers to the data receiver object - creating either fragments
  // or raw buffers depending on raw buffer mode
  DAQLogger::LogDebug(instance_name_) << "Pre-committing "
                                      << raw_buffer_precommit_
                                      << " buffers of size " << raw_buffer_size_
                                      << " to receiver";
  ...
  for (unsigned int i = 0; i < raw_buffer_precommit_; i++) {
    ...
    // DAQLogger::LogDebug(instance_name_) << "Pre-commiting raw buffer " << i
    // << " at address " << (void*)(raw_buffer->dataPtr());
  ...
  std::ostringstream config_frag;
  config_frag << "<DataDpm><DataBuffer><RunMode>" << rce_daq_mode_
              << "</RunMode></DataBuffer></DataDpm>";
  dpm_client_->send_config(config_frag.str());

  // Set the number of microslices_per_trigger
  std::ostringstream trigsize_frag;
  trigsize_frag << "<DataDpm><DataBuffer><TrgAcceptCnt>" << rce_trg_accept_cnt_
                << "</TrgAcceptCnt></DataBuffer></DataDpm>";
  dpm_client_->send_config(trigsize_frag.str());

  // this sets  the software trigger frequency...needs to go away once we have
  // real trigger
  std::ostringstream frmcnt_frag;
  frmcnt_frag << "<DataDpm><DataBuffer><TrgFrameCnt>" << rce_trg_frame_cnt_
              << "</TrgFrameCnt></DataBuffer></DataDpm>";
...
// dpm_client_->send_command("STOP");
  ...
  DAQLogger::LogInfo(instance_name_)
      << "Low water mark on empty buffer queue is " << empty_buffer_low_mark_;
  DAQLogger::LogInfo(instance_name_)
      << "High water mark on filled buffer queue is "
      << filled_buffer_high_mark_;
  ...
  double data_rate_mbs =
      ((double)total_bytes_received_) / ((1024 * 1024) * elapsed_secs);

  DAQLogger::LogInfo(instance_name_) << "Received " << millislices_received_
                                     << " millislices in " << elapsed_secs
                                     << " seconds, rate " << rate
                                     << " Hz, total data "
                                     << total_bytes_received_ << " bytes, rate "
                                     << data_rate_mbs << " MB/s" << std::endl;
}

void dune::TpcRceReceiver::stopNoMutex(void) {

  DAQLogger::LogInfo(instance_name_)
      << "In stopNoMutex - instructing DPM to stop";
...
bool dune::TpcRceReceiver::getNext_(artdaq::FragmentPtrs& frags) {
  ...
  // allowed
  while ((data_receiver_->filled_buffers_available() > 0) &&
         (buffers_released <= filled_buffer_release_max_)) {
    bool buffer_available = data_receiver_->retrieve_filled_buffer(
        recvd_buffer, buffer_recv_timeout_us);

    if (!buffer_available) {
      DAQLogger::LogWarning(instance_name_)
          << "dune::TpcRceReceiver::getNext_ : "
          << "receiver indicated buffers available but unable to retrieve one "
             "within timeout";
      continue;
    }
    ...
    if (recvd_buffer->size() == 0) {
      DAQLogger::LogWarning(instance_name_)
          << "dune::TpcRceReceiver::getNext_ : no data received in raw buffer";
      continue;
    }
    ...
    if (use_fragments_as_raw_buffer_) {
      // Map back onto the fragment from the raw buffer data pointer we have
      // just received
      if (raw_to_frag_map_.count(data_ptr)) {
        ...
        millislice_size = validate_millislice_from_fragment_buffer(
            frag->dataBeginBytes(), recvd_buffer->size(),
            recvd_buffer->count());

        // Clean up entry in map to remove raw fragment buffer
        raw_to_frag_map_.erase(data_ptr);

        // Create a new raw buffer pointing at a new fragment and replace the
        // received buffer
        ...
        DAQLogger::LogError(instance_name_)
            << "dune::TpcRceReceiver::getNext_ : cannot map raw buffer with "
               "data address" << (void*)recvd_buffer->dataPtr()
            << " back onto fragment";
        continue;
      }
    }
    // Otherwise format the raw buffer into a new fragment
      ...
      // Create an artdaq::Fragment to format the raw data into. As a crude
      // heuristic,
      // reserve 10 times the raw data space in the fragment for formatting
      // overhead.
      ...
      // Format the raw digitisations (nanoslices) in the received buffer into a
      // millislice
      // within the data payload of the fragment
      millislice_size = format_millislice_from_raw_buffer(
          (uint16_t*)data_ptr, recvd_buffer->size(),
    ...
    if (max_rce_events_ == 0 || ev_counter() <= max_rce_events_) {
      ...
      // We've already got as much data as we want downstream, so just send a
      // header
      frag->resizeBytes(0);
    }
  ...
  if (reporting_interval_time_ != 0) {
    ...
    if (std::chrono::duration_cast<std::chrono::milliseconds>(
            now - report_time_).count() > (reporting_interval_time_ * 1000)) {
      auto elapsed_msecs =
      ...
      DAQLogger::LogInfo(instance_name_)
          << "dune::TpcRceReceiver::getNext_ : received "
          << millislices_received_ << " millislices, "
          << float(total_bytes_received_) / (1024 * 1024) << " MB in "
  ...
  if (should_stop()) {
    if (data_receiver_->filled_buffers_available() > 0) {
      // Don't signal board reader can stop if there are still buffers available
      // to release as fragments
      DAQLogger::LogDebug(instance_name_) << "dune::TpcRceReceiver::getNext_ : "
                                             "should_stop() is true but "
                                             "buffers available";
    } else {
      // If all buffers released, set is_active false to signal board reader can
      // stop
      DAQLogger::LogInfo(instance_name_) << "dune::TpcRceReceiver::getNext_ : "
                                            "no unprocessed filled buffers "
                                            "available at end of run";
      is_active = false;
    }
  }
  if (data_receiver_->exception()) {
    set_exception(true);
    DAQLogger::LogError(instance_name_) << "dune::TpcRceReceiver::getNext_ : "
                                           "found receiver thread in exception "
                                           "state";
  }
  ...
  if (!buffers_found_in_while_loop) {
    ...
    auto time_since_last_buffer =
        std::chrono::duration_cast<std::chrono::microseconds>(
            std::chrono::high_resolution_clock::now() -
            last_buffer_received_time_).count();

    if (time_since_last_buffer > data_timeout_usecs_) {
      DAQLogger::LogError(instance_name_)
          << "dune::TpcRceReceiver::getNext_ : timeout due to no data "
             "appearing after " << time_since_last_buffer / 1000000.0
          << " seconds";
    }
  }
...
bool dune::TpcRceReceiver::getNext_(artdaq::FragmentPtrs& frags) {
  ...
  // Wait for a filled buffer to be available, with a timeout tick to allow the
  // should_stop() state
  // to be checked
  do {
    buffer_available =
        data_receiver_->retrieve_filled_buffer(recvd_buffer, 500000);
    if (reporting_interval_time_ != 0) {

      if (std::chrono::duration_cast<std::chrono::milliseconds>(
              std::chrono::high_resolution_clock::now() - report_time_)
              .count() > (reporting_interval_time_ * 1000)) {
        report_time_ = std::chrono::high_resolution_clock::now();
        DAQLogger::LogInfo(instance_name_)
            << "Received " << millislices_received_ << " millislices so far";
      }
    }
  ...
  if (should_stop() || data_receiver_->exception()) {
    ...
    if (data_receiver_->filled_buffers_available() > 0) {
      DAQLogger::LogWarning(instance_name_) << "getNext_ stopping while there "
                                               "were still filled buffers "
                                               "available";
    } else {
      DAQLogger::LogInfo(instance_name_)
          << "No unprocessed filled buffers available at end of run";
    }
    ...
    if (data_receiver_->exception()) {
      ...
      throw std::runtime_error(
          "RceDataReceiver object found in exception state");
    }
  ...
  if (recvd_buffer->size() == 0) {
    DAQLogger::LogWarning(instance_name_)
        << "getNext_ : no data received in raw buffer";
    return true;
  }
  ...
  if (use_fragments_as_raw_buffer_) {
    // Map back onto the fragment from the raw buffer data pointer we have just
    // received
    if (raw_to_frag_map_.count(data_ptr)) {
      // Extract the fragment from the map
      frag = std::move(raw_to_frag_map_[data_ptr]);
      ...
      // Validate and finalize the fragment received
      millislice_size = validate_millislice_from_fragment_buffer(
          frag->dataBeginBytes(), recvd_buffer->size(), recvd_buffer->count());
      ...
      DAQLogger::LogError(instance_name_)
          << "Cannot map raw buffer with data address"
...
dune::RceRawBufferPtr dune::TpcRceReceiver::create_new_buffer_from_fragment(
    void) {
  RceRawBufferPtr raw_buffer;

  std::unique_ptr<artdaq::Fragment> frag = artdaq::Fragment::FragmentBytes(
      raw_buffer_size_ + sizeof(TpcMilliSlice::Header));
  uint8_t* data_ptr = frag->dataBeginBytes() + sizeof(TpcMilliSlice::Header);
  raw_buffer =
      dune::RceRawBufferPtr(new RceRawBuffer(data_ptr, raw_buffer_size_));

  raw_to_frag_map_.insert(
      std::pair<uint8_t*, std::unique_ptr<artdaq::Fragment> >(
          raw_buffer->dataPtr(), std::move(frag)));

  return raw_buffer;
}

uint32_t dune::TpcRceReceiver::format_millislice_from_raw_buffer(
    uint16_t* src_addr, size_t src_size, uint8_t* dest_addr, size_t dest_size) {
  ...
  for (uint32_t udx = 0; udx < number_of_microslices_to_generate_; ++udx) {
    ...
    if (microslice_writer_ptr.get() == 0) {
      ...
      for (uint32_t ndx = 0; ndx < number_of_nanoslices_to_generate_; ++ndx) {
        nanoslice_writer_ptr =
            microslice_writer_ptr->reserveNanoSlice(NANOSLICE_BUFFER_SIZE);
  ...
  if (sample_addr > (src_addr + src_size)) {
    DAQLogger::LogError(instance_name_)
        << "Raw buffer overrun during millislice formatting by "
        << ((src_addr + src_size) - sample_addr);
  }
  millislice_writer.finalize();
  return millislice_writer.size();
}

uint32_t dune::TpcRceReceiver::validate_millislice_from_fragment_buffer(
    uint8_t* data_addr, size_t data_size, uint32_t count) {

  dune::TpcMilliSliceWriter millislice_writer(
      data_addr, data_size + sizeof(TpcMilliSlice::Header));
