...
////////////////////////////
 ...
 DAQLogger::LogInfo("PennReceiver") << "Debug level set to " << receiver_debug_level;
  ...
  penn_client_ = std::unique_ptr<dune::PennClient>(new dune::PennClient(
  ...
  if (penn_client_->exception()) {
    set_exception(true);
    DAQLogger::LogError("PennReceiver") << "dune::PennReceiver::constructor_ : found penn client in exception state";
  }
  sleep(1);
  ...
  data_receiver_ =
      std::unique_ptr<dune::PennDataReceiver>(new dune::PennDataReceiver(receiver_debug_level,
                                                                         receiver_tick_period_usecs_,
                                                                         penn_data_dest_port_,
                                                                         millislice_size_,
                                                                         millislice_overlap_size_,
...
// Sleep for a short while to give time for the DataReceiver to be ready to
  ...
  if (penn_client_->exception()) {
    set_exception(true);
    DAQLogger::LogError("PennReceiver") << "dune::PennReceiver::constructor_ : found control thread in exception state";
  }
}

void dune::PennReceiver::start(void)
{
#ifdef __PTB_DEBUG__
  ...
  {
    dune::PennRawBufferPtr raw_buffer;
    ...
    else
    {
      ...
      raw_buffer = dune::PennRawBufferPtr(new PennRawBuffer(raw_buffer_size_));
    }
#ifdef __PTB_DEBUG__
  ...
  if (penn_client_->exception()) {
    set_exception(true);
    DAQLogger::LogError("PennReceiver") << "dune::PennReceiver::start_ : found control thread in exception state";
  }
...
void dune::PennReceiver::stopNoMutex(void)
{
...
void dune::PennReceiver::stop(void)
{
...
#ifndef OLD_GETNEXT
bool dune::PennReceiver::getNext_(artdaq::FragmentPtrs & frags) {
	...
	while ((data_receiver_->filled_buffers_available() > 0) && (buffers_released <= filled_buffer_release_max_))
	{
		...
		if (!buffer_available)
		{
			DAQLogger::LogWarning("PennReceiver") << "dune::PennReceiver::getNext_ : "
		...
		{
			DAQLogger::LogWarning("PennReceiver") << "dune::PennReceiver::getNext_ : no data received in raw buffer";
			continue;
		}
		...
		if (use_fragments_as_raw_buffer_)
		{
			...
			{
				DAQLogger::LogError("PennReceiver") << "dune::PennReceiver::getNext_ : cannot map raw buffer with data address"
	...
	if (reporting_interval_time_ != 0)
	{
			...
			(reporting_interval_time_ * 1000))
		{
			...
			DAQLogger::LogInfo("PennReceiver") << "dune::PennReceiver::getNext_ : received " << millislices_received_ << " millislices, "
	...
	artdaq::Globals::metricMan_->sendMetric("PTB Empty Buffers Available", empty_buffers_available, "buffers", 1, artdaq::MetricMode::Accumulate);
	artdaq::Globals::metricMan_->sendMetric("PTB Filled Buffer High Water Mark", filled_buffer_high_mark_, "buffers", 1, artdaq::MetricMode::Accumulate);
	artdaq::Globals::metricMan_->sendMetric("PTB Filled Buffers Availiable", filled_buffers_available, "buffers", 1, artdaq::MetricMode::Accumulate);
	...
	if (should_stop())
	{
		if (data_receiver_->filled_buffers_available() > 0)
		{
			...
			DAQLogger::LogDebug("PennReceiver")
				<< "dune::PennReceiver::getNext_ : should_stop() is true but buffers available";
		}
		else
		{
			// If all buffers released, set is_active false to signal board reader can stop
			DAQLogger::LogInfo("PennReceiver")
				<< "dune::PennReceiver::getNext_ : no unprocessed filled buffers available at end of run";
			is_active = false;
		}
	}
	...
	{
	  set_exception(true);
	  DAQLogger::LogError("PennReceiver") << "dune::PennReceiver::getNext_ : found receiver thread in exception state";
	}
	...
	if ( ! buffers_found_in_while_loop ) {
	  ...
	  if (time_since_last_buffer > data_timeout_usecs_) {
	    DAQLogger::LogError("PennReceiver") << "dune::PennReceiver::getNext_ : timeout due to no data appearing after " << time_since_last_buffer / 1000000.0 << " seconds";
	  }
	}
...
// Update buffer high and low water marks
  ...
  {     // Only do the metrics at the reporting intervals, the RCEs do it every millislice 
    artdaq::Globals::metricMan_->sendMetric(empty_buffer_low_water_metric_name_, empty_buffer_low_mark_, "buffers", 1, artdaq::MetricMode::Accumulate);
    artdaq::Globals::metricMan_->sendMetric(empty_buffer_available_metric_name_, empty_buffers_available, "buffers", 1, artdaq::MetricMode::Accumulate);
    artdaq::Globals::metricMan_->sendMetric(filled_buffer_high_water_metric_name_, filled_buffer_high_mark_, "buffers", 1, artdaq::MetricMode::Accumulate);
    artdaq::Globals::metricMan_->sendMetric(filled_buffer_available_metric_name_, filled_buffers_available, "buffers", 1, artdaq::MetricMode::Accumulate);
  }
...
dune::PennRawBufferPtr dune::PennReceiver::create_new_buffer_from_fragment(void)
{
  ...
  raw_buffer = dune::PennRawBufferPtr(new PennRawBuffer(data_ptr, raw_buffer_size_));
...
#ifndef REBLOCK_PENN_USLICE
uint32_t dune::PennReceiver::format_millislice_from_raw_buffer(uint16_t* src_addr, size_t src_size,
...
uint32_t dune::PennReceiver::validate_millislice_from_fragment_buffer(uint8_t* data_addr, size_t data_size,
#ifndef REBLOCK_PENN_USLICE
    uint32_t us_count,
#endif
...
void dune::PennReceiver::generate_config_frag(std::ostringstream& config_frag) {
...
void dune::PennReceiver::generate_config_frag_emulator(std::ostringstream& config_frag) {
...
bool dune::PennReceiver::startOfDatataking() {
...
DEFINE_ARTDAQ_COMMANDABLE_GENERATOR(dune::PennReceiver) 
