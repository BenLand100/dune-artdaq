...
dune::RootApplication::RootApplication (fhicl::ParameterSet const &ps): art::EDAnalyzer(ps), force_new_(ps.get<bool> ("force_new", true)), dont_quit_(ps.get<bool> ("dont_quit", false)) {}

dune::RootApplication::~RootApplication () { }

void dune::RootApplication::analyze (art::Event const &) {
  gSystem->ProcessEvents();
}

void dune::RootApplication::beginJob () { 
...
void dune::RootApplication::endJob() {
  if (dont_quit_) app_->Run (true);
}

DEFINE_ART_MODULE(dune::RootApplication)
