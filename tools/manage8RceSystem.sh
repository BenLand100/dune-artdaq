...
function launch() {
  ...
  if [[ "${12}" == "1" ]]; then
   ...
   DemoControl.rb ${enableSerial} -s -c $1 \
     --tpc `hostname`,${DUNEARTDAQ_BR_PORT[0]},0 \
     --tpc `hostname`,${DUNEARTDAQ_BR_PORT[1]},1 \
     --tpc `hostname`,${DUNEARTDAQ_BR_PORT[2]},2 \
     --tpc `hostname`,${DUNEARTDAQ_BR_PORT[3]},3 \
     --tpc `hostname`,${DUNEARTDAQ_BR_PORT[4]},4 \
     --tpc `hostname`,${DUNEARTDAQ_BR_PORT[5]},5 \
     --tpc `hostname`,${DUNEARTDAQ_BR_PORT[6]},6 \
     --tpc `hostname`,${DUNEARTDAQ_BR_PORT[7]},7 \
...
logFile="/data/dunedaq/daqlogs/masterControl/dsMC-${TIMESTAMP}-${command}.log"
