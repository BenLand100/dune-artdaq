...
int main(int argc, char * argv[]) try
{
  ...
  while (events_to_generate >=0 && gen->getNext(frags)) {
    ...
    for (auto & val : frags) {
      ...
      if (val->sequenceID() - previous_sequence_id > 1) {
	throw cet::exception("Event skipped; non-consecutive sequence IDs seen");
      }
  ...
  if (event_count != events_to_generate) {
    throw cet::exception("Mismatch between expected and generated number of events");
  }
  ...
  if (! endSucceeded) {
    throw cet::exception("Failed to shut down the reader and the event store because the endOfData marker could not be pushed onto the queue.");
  }
